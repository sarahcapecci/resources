<?php
include_once('class.wpmessenger.list_table.php');
class WPMessenger_List_Inbox extends WPMessenger_List_Table {
    ...
    function __construct(){
        ...
        parent::__construct( array(
            ...
            'plural'    => 'Inbox', //plural name of the listed records
    ...
    function column_id($item){
        ...
        $actions = array(
            'delete'    => sprintf('<a href="wpmessenger_op=delete&level=inbox&record_id=%s" onclick="return confirm(\'Are you sure you want to delete this entry?\')">Delete</a>',$item['id']),
        );
...
//        );
    ...
    function process_bulk_action() {        
        ...
        if( 'delete'===$this->current_action() ) {
        	$records_to_delete = $_POST['inbox'];//value of $this->_args['singular'] is the "key"
        	...
        	foreach ($records_to_delete as $id){
			    global $wpdb;
				$updatedb = "DELETE FROM $r WHERE msg_id='$id' AND recipient_id=" .$userInfo->ID;
    ...
    function prepare_items() {
        ...
        $m = $this->conf->msgtbl;
        $r = $this->conf->recipients;                            
        $query = "SELECT ".$m.".id, subject,user_id,display_name, received FROM ". $r .
                " LEFT JOIN " . $m.
                " ON (".$r.".msg_id = ".$m.".id)".
                " LEFT JOIN " . $wpdb->users . " ON (".$wpdb->users.".ID = user_id)".
                " WHERE " . $r . ".recipient_id=" . $userInfo->ID.
                " AND ". $r.".label='inbox' ORDER BY $orderby_column $sort_order";
