...
class WPMessenger_List_Sent extends WPMessenger_List_Table {
    ...
    function __construct(){
        ...
        parent::__construct( array(
            'singular'  => 'Sent', //singular name of the listed records
            'plural'    => 'Sent', //plural name of the listed records
...
//        );
    ...
    function process_bulk_action() {        
        ...
        if( 'delete'===$this->current_action() ) {
        	$records_to_delete = $_POST['sent'];//value of $this->_args['singular'] is the "key"
        	...
        	foreach ($records_to_delete as $id){
				...
				$fields = array('state'=>'deleted');
				$results = $wpdb->update($m, $fields, array('id'=>$id,'user_id'=>$userInfo->ID));        		
        	}
    ...
    function prepare_items() {
		...
		*/
        ...
        $query =" SELECT ".$m.".id, subject, recipient_id, display_name, created FROM ". $r .
                " LEFT JOIN " . $m.
                " ON (".$r.".msg_id = ".$m.".id)".
                " LEFT JOIN " . $wpdb->users . " ON (".$wpdb->users.".ID = recipient_id)".
                " WHERE " . $m . ".user_id=" . $userInfo->ID.
                " AND ". $m.".state='sent' ORDER BY $orderby_column $sort_order";
